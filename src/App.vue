  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span
                v-for="possibleTicker in possibleTickers"
                :key="possibleTicker"
                @click="addTicker(possibleTicker.Name)"
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer"
              >
                {{ possibleTicker.Name }}
              </span>
            </div>
            <div class="text-sm text-red-600" v-show="tickerAlreadyAdded">
      ...
      <section v-if="selectedTicker" class="relative">
        ...
        <button @click="selectTicker(null)" type="button" class="absolute top-0 right-0">
          <svg
            xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  methods: {
    ...
    getBarHeight(price) {
      ...
      const diff = Math.max(...this.graph) - min;
      const value = price - min;
      if (Number.isFinite(diff)) {
        const result = 5 + (value * 95) / diff;
        return result;
      }
      return 50;
    },
